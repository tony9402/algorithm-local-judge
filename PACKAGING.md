...
이 문서는 사용자가 Python 설치 없이 `judge` 명령을 사용할 수 있도록 standalone
실행 파일, Debian package, Homebrew formula, container image와 problem pack을 만드는
방법을 정리한다.

최종 사용자의 GitHub clone 설치는 이 문서가 아니라 [개인 설치 및 운영 안내](INSTALL.md)를
따릅니다. 이 문서의 packaging 명령은 release 담당자와 빌드 환경을 위한 것입니다.
...
- Release build는 `.github/workflows/release.yml`의 플랫폼별 GitHub Actions를 기준으로 한다.
- 지원 platform id는 `macos-arm64`, `macos-amd64`, `linux-amd64`,
  `windows-amd64`이다. 네이티브 설치 형식은 macOS PKG, Debian/APT, RPM,
  Windows MSI/WinGet을 대상으로 한다.
...
공식 `.aljpack` release asset은 checksum sidecar와 Sigstore bundle을 함께 게시해야 한다.
...
직접 HTTP(S) `.aljpack` URL 설치는 사용자가 URL을 명시적으로 신뢰한 로컬 설치 경로로
취급한다. 공식 배포에는 direct URL 대신 trusted GitHub release asset과 `.sha256`,
`.sigstore.json` sidecar를 사용한다. Judge는 공식 GitHub pack의 repository/tag workflow
identity와 GitHub Actions OIDC issuer를 Cosign으로 검증한 뒤에만 설치한다.

팩 서명과 검증:

```bash
judge pack sign dist/packs/basic-1-macos-arm64.aljpack
judge pack verify-signature \
  dist/packs/basic-1-macos-arm64.aljpack \
  --repository owner/name
```

공식 릴리스는 keyless Sigstore bundle을 사용한다. 사설·오프라인 환경은
`ALJ_PACK_SIGNATURE_PUBLIC_KEY=/path/to/cosign.pub`로 고정 공개키 정책을 사용할 수 있다.
Cosign 경로는 `ALJ_COSIGN`으로 지정한다.
...
후보 채널은 인증정보 없이 설치 구조를 검사할 수 있는 unsigned PKG와
`*.native-signing.json`을 만들며, 이 증거의 상태는 `unconfigured`이다.

```bash
uv run python scripts/build_macos_pkg.py \
  --archive dist/standalone/algorithm-local-judge-0.1.0-macos-arm64.tar.gz \
  --platform macos-arm64 --channel candidate
```

stable 채널은 `APPLE_DEVELOPER_ID_APPLICATION`,
`APPLE_DEVELOPER_ID_INSTALLER`, `APPLE_NOTARY_PROFILE`을 모두 요구한다. 중첩된 모든
Mach-O 파일의 inside-out hardened-runtime 서명, Installer 서명, `notarytool`의
`Accepted` 결과, stapling 및 Gatekeeper 평가 중 하나라도 실패하면 PKG를 stable
산출물로 인정하지 않는다. 경로 탈출이나 링크가 있는 입력 archive도 staging 전에
거부한다.

### 10.2 Debian APT 계약

후보 채널은 공개 URL이나 키를 만들어 내지 않고 unsigned `Release` 메타데이터만
검사한다.

```bash
uv run python scripts/build_apt_repository.py \
  --deb dist/installers/algorithm-local-judge_0.1.0_amd64.deb \
  --channel candidate
```

stable 채널은 `ALJ_APT_GPG_KEY_ID`, 실제 HTTPS 저장소 URL, 배포할 공개키가 모두
필요하다. 빌더는 `Release.gpg`와 `InRelease`를 생성한 후 GPG로 다시 검증하고,
공개키와 `Signed-By` Deb822 source만 소유하는 bootstrap DEB를 별도로 만든다. 비밀키와
공개 주소는 저장소에 기록하지 않으며 GitHub Actions secret/variable이 없으면 태그
빌드가 즉시 실패한다.

```bash
uv run python scripts/build_apt_repository.py \
  --deb dist/installers/algorithm-local-judge_0.1.0_amd64.deb \
  --channel stable --gpg-key-id "$ALJ_APT_GPG_KEY_ID" \
  --repository-url "$APT_REPOSITORY_URL" --public-key "$APT_GPG_PUBLIC_KEY"
```

Homebrew formula의 설치 검증은 두 launcher의 `--help`뿐 아니라 네트워크 설치나 상태
변경을 하지 않는 `judge setup --check-only`도 실행한다.
...
dist/packs/basic-1-macos-arm64.aljpack
dist/packs/basic-1-macos-arm64.aljpack.sha256
dist/packs/basic-1-macos-arm64.aljpack.sigstore.json
dist/installers/algorithm-local-judge_0.1.0_amd64.deb
dist/installers/algorithm-local-judge-0.1.0-macos-arm64.pkg
dist/installers/algorithm-local-judge-0.1.0-apt-repository.tar.gz
dist/installers/algorithm-local-judge-archive-keyring_0.1.0_all.deb
algorithm-local-judge.cdx.json
```

현재 native stable 채널은 clean-OS 설치·업그레이드·rollback 증거가 모두 게시되기 전까지
미공개 상태다. 최종 사용자는 검증되지 않은 raw branch 설치 스크립트를 실행하지 말고
[개인 설치 및 운영 안내](INSTALL.md)의 GitHub clone 설치 절차를 사용한다. native 채널을
공개할 때에는 tag와 release manifest를 고정한 설치 명령을 이 섹션에 추가한다.
...
Stable 릴리스는 `release-manifest.json`에 다음 항목을 고정하고 로컬 검증을 통과해야 한다.

- release version/tag와 40자리 source commit
- 지원 OS별 standalone artifact와 `judge`, `problem-studio` launcher
- 각 artifact의 SHA-256 sidecar와 Sigstore bundle
- CycloneDX SBOM과 SBOM sidecar
- commit으로 고정된 공식 problem pack repository/ref/asset/hash/signature
- macOS Developer ID/notarization, Windows Authenticode, APT/RPM GPG 검증 attestation

후보 manifest 생성과 검증:

```bash
python scripts/build_native_signing_plan.py --output release-assets/native-signing-plan.json
python scripts/build_release_manifest.py \
  --assets release-assets \
  --output release-assets/release-manifest.json \
  --tag v0.1.0 \
  --channel candidate \
  --source-commit local-candidate \
...
`build_native_signing_plan.py`는 서명을 수행하거나 비밀 값을 출력하지 않는다. 다음 자격증명과
도구의 준비 여부만 기록하며, 누락 상태는 `unconfigured`이다.

| 대상 | 자격증명 환경 변수 | 필요한 도구 | stable 증명 |
|---|---|---|---|
| macOS | `APPLE_DEVELOPER_ID_APPLICATION`, `APPLE_NOTARY_PROFILE` | `codesign`, `xcrun` | notarization attestation |
| Windows | `WINDOWS_SIGN_CERTIFICATE`, `WINDOWS_SIGN_PASSWORD` | `signtool` | Authenticode verification attestation |
| APT | `ALJ_APT_GPG_KEY_ID` | `gpg` | signed repository attestation |
| RPM | `ALJ_RPM_GPG_KEY_ID` | `gpg`, `rpmsign` | package/repository verification attestation |

자격증명이나 실제 attestation이 없으면 candidate 검증까지만 가능하다. `--stable` 검증은
native signing, immutable official pack reference 또는 manifest 자체의 checksum/Sigstore
sidecar 중 하나라도 빠지면 게시 전에 실패한다. PR, fork, 수동 non-tag 실행은 release
workflow의 publish job을 실행하지 않는다.

키 폐기·교체와 N-1 복구 절차는
[`packaging/SIGNING_KEY_ROTATION.md`](packaging/SIGNING_KEY_ROTATION.md)를 따른다. 새 identity와
N/N-1 lifecycle smoke가 검증되기 전에는 stable pointer나 설치 명령을 변경하지 않는다.

게시 후에는 새 디렉터리에 release asset을 다시 내려받아 다음 smoke를 수행한다.

```bash
python scripts/smoke_release_availability.py --assets published-assets
```

이 smoke는 manifest가 참조한 실제 artifact와 모든 sidecar를 다시 해시 검증한다.
