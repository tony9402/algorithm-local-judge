        ...
        with isolated_runtime("alj-problem-studio-bulk-gap-e2e-") as (_directory, workspace):
            ...
            with patch(
                ...
                with run_app(create_app(workspace)) as server:
                    ...
                    wait_for_text(page, "#problemList", "문제 있음")
                    page.locator('[data-tab="build"]').click()
                    self.assertFalse(page.locator("#buildDiagnostics").is_visible())
                    page.locator("#problemList .list-item").filter(has_text="Beta Bulk").click()
                    wait_for_text(page, "#problemTitle", "Beta Bulk")
                    page.locator('[data-tab="build"]').click()
