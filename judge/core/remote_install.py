...
from judge.core.pack_signatures import verify_pack_signature
...
def download_problem_pack_from_github(
    ...
    Args:
        ...
        require_pack (bool): release pack이 없을 때 source archive fallback을 금지할지 결정합니다.
    ...
    if not ref:
        try:
            ...
            if asset_name or require_pack:
                if require_pack:
                    raise JudgeError(
                        f"signed release problem pack is required for {repo}"
                    ) from exc
                raise
        else:
            ...
            signature_asset = select_signature_asset(release.get("assets", []), asset)
            signature_url = signature_asset.get("browser_download_url")
            if not isinstance(signature_url, str):
                raise JudgeError(
                    f"problem pack signature has no download URL: {signature_asset.get('name')}"
                )
            ...
            signature_target = target_dir / safe_download_name(
                signature_asset.get("name"),
                f"{target.name}.sigstore.json",
            )
            download_asset(download_url, target)
            download_asset(checksum_url, checksum_target)
            download_asset(signature_url, signature_target)
            try:
                checksum = verify_sha256_text(
                    target,
                    checksum_target.read_text(encoding="utf-8"),
                )
                signature = verify_pack_signature(target, signature_target, repo)
            except Exception:
            ...
            result.update(
                {
                    ...
                    "signatureSource": signature_asset.get("name"),
                    **signature,
                }
            )
            return result
...
def download_problem_pack_from_url(
    ...
    *,
    ...
    if signature_url:
        signature_parsed = urlparse(signature_url)
        if signature_parsed.scheme not in {"http", "https"}:
            raise JudgeError("direct problem pack signature URL must be HTTP(S)")
    elif require_signature:
        signature_url = f"{source_url}.sigstore.json"
    target_dir = cache_root() / "downloads" / "packs"
    target_dir.mkdir(parents=True, exist_ok=True)
    ...
    signature_target: Path | None = None
    ...
    signature_result: dict[str, Any] = {
        ...
        "signaturePolicy": "checksum-only; publisher signature not provided",
    }
    if signature_url:
        signature_target = target.with_name(
            safe_download_name(
                Path(urlparse(signature_url).path).name,
                f"{target.name}.sigstore.json",
            )
        )
        try:
            download_asset(signature_url, signature_target)
            signature_result = {
                **verify_pack_signature(target, signature_target, "direct-url"),
                "signatureSource": signature_url,
            }
        except Exception:
            target.unlink(missing_ok=True)
            if checksum_target is not None:
        ...
        if checksum_target is not None:
            checksum_target.unlink(missing_ok=True)
        raise JudgeError("direct problem pack URL requires a publisher signature")
    result = install_downloaded_problem_pack(target)
    result.update(
        {
